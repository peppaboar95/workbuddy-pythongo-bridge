...
TOOLS = [
    ...
    _tool("get_trade_intent", "Read an intent, child orders and linked trades.", _object({"intent_id": STRING}, ["intent_id"])),
    ...
    _tool("request_sync", "Request fresh Adapter snapshots. This never creates a trade intent.", _object({
        "account_alias": ACCOUNT,
    ...
    _tool("submit_trade_intent", "Consume one unexpired preview after recomputing risk and authorization.", _object({"preview_id": STRING, "approval_context": _object({"local_approval_id": STRING})}, ["preview_id"]), read_only=False, idempotent=True),
