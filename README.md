...
首次需要交易时，完成 P0/Profile 验证后，先在启动入口选择 `1` 运行观察模式并保持窗口打开，在无限易中启动 Adapter，再另开状态入口检查。前置条件不足时会列出具体步骤；全部通过后才显示 `T：确认首次启用交易`，明确确认后解除首次安装保护，仍保持观察模式。Worker 已运行时，再次打开启动入口也可查看这项检查。完整安装、P0、模式切换、日常运行和故障处理请阅读 [操作手册](docs/README-RELEASE.zh-CN.md)。
...
- [0.3.9 发布说明](docs/RELEASE-v0.3.9.md)
