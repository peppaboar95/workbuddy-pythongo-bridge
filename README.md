...
首次需要交易时，在现有启动入口输入 `T` 进入“首次启用交易”向导；Worker 已运行时，启动或状态窗口同样提供此入口。向导列出待完成的 P0/Profile 和 Adapter 检查，检查通过并明确确认后才解除首次安装保护，仍保持观察模式。完整安装、P0、模式切换、日常运行和故障处理请阅读 [操作手册](docs/README-RELEASE.zh-CN.md)。
