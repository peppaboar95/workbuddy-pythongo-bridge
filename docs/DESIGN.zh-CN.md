...
> 状态：Design v0.3.9（对齐 WorkBuddy-QMT Bridge 0.3.0 的 `LIMITED_AUTO` 安全模型）
> 日期：2026-09-13
...
6. 确认自动同步已完成，核对资金和持仓；委托与成交以回调和执行日记为准，目标合约行情按需调用 `request_sync`；
7. 完成必要对账后才评估非观察模式。

上述第 2、3 步可交换，也可先启动 Worker 再打开无限易。Worker 只在 Adapter 为 READY、心跳新鲜且模式和保证金策略一致时发起只读同步，离线时等待，不提前投递会过期的启动同步命令。每个 Worker 进程首次连接、断线恢复及 Adapter 会话变化后都重新同步，等待回执和请求后的资金、持仓快照；失败按间隔重试，不循环堆积有效请求。心跳使用发生时间判断新鲜度，较旧心跳不得覆盖较新的状态；过期但通过签名、结构及账号绑定校验的心跳仅作为历史记录接收，不产生新连接，其他命令和事件仍严格拒绝过期消息。自动连接和同步不得解除保护、创建交易授权或推断未知委托已完成。
