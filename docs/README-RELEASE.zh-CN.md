# WorkBuddy-PythonGO Bridge 0.3.9 操作手册
...
若旧桌面入口仍显示中文乱码，或提示中文文本“不是内部或外部命令”，重新运行本版本安装器并选择创建桌面入口即可更新。只会生成启动和状态两个独立的 `.cmd` 文件；中文处理逻辑内嵌在入口中，无需旁边的 `.ps1` 文件。更新入口的备份和旧版本生成的辅助脚本会保存在运行目录的 `config/launcher-backups` 中。
